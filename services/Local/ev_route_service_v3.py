"""
EV Route Service V3 - Dynamic Efficiency with Strategic Battery Utilization 
...
- Fixed cache format compatibility issues
"""
...
# Initialize empty station structures
STATION_MAPPING: Dict[str, str] = {}
DEFAULT_CHARGING_STATIONS_COORDS: List[List[float]] = []
    ...
    with open(STATIONS_CSV_FILE, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            try:
                ...
                # Update mapping and coordinates list
                STATION_MAPPING[f"({lat},{lon})"] = name
                DEFAULT_CHARGING_STATIONS_COORDS.append([lat, lon])
            except ValueError as e:
                print(f"Skipping invalid row in stations.csv: {row} ({e})")
    print(f"Loaded {len(DEFAULT_CHARGING_STATIONS_COORDS)} charging stations from stations.csv")
...
# Cache configuration
CACHE_DIR = "cache"
DISTANCE_CACHE_FILE = os.path.join(CACHE_DIR, "google_distances_v3.pkl")
ELEVATION_CACHE_FILE = os.path.join(CACHE_DIR, "elevation_factors_v3.pkl")
CONDITIONS_CACHE_FILE = os.path.join(CACHE_DIR, "dynamic_conditions_v3.pkl")
...
def perpendicular_distance(point, line_start, line_end):
        """
        Calculate perpendicular distance from 'point' to the line defined by line_start -> line_end
        All points are (lat, lon) tuples.
        Returns distance in same units as coordinates (can multiply by km factor if needed).
        """
        x0, y0 = point
        x1, y1 = line_start
        x2, y2 = line_end
...
class DynamicEfficiencyManager:
    ...
    def _load_elevation_cache(self):
        """Load static elevation factors from cache with format validation"""
        try:
            if os.path.exists(ELEVATION_CACHE_FILE):
                ...
                if isinstance(cache_data, dict) and 'data' in cache_data:
                    ...
                    raw_cache = cache_data.get('data', {})
                    self.elevation_cache = self._validate_elevation_cache(raw_cache)
                    print(f"Loaded {len(self.elevation_cache)} elevation factors from cache")
                elif isinstance(cache_data, dict):
                    # Direct cache data (older format)
                    self.elevation_cache = self._validate_elevation_cache(cache_data)
                    print(f"Loaded {len(self.elevation_cache)} elevation factors from cache (old format)")
                else:
                    print("Invalid elevation cache format, starting fresh")
                    self.elevation_cache = {}
        except Exception as e:
            print(f"Warning: Could not load elevation cache: {e}")
            self.elevation_cache = {}
    
    def _validate_elevation_cache(self, raw_cache: dict) -> dict:
        """Validate and convert elevation cache entries to consistent format"""
        validated_cache = {}
        converted_count = 0
        
        for key, value in raw_cache.items():
            try:
                if isinstance(value, dict) and 'elevation_factor' in value:
                    # Already in correct format
                    validated_cache[key] = value
                elif isinstance(value, (int, float)):
                    # Old format - convert to new format
                    validated_cache[key] = {
                        'elevation_factor': float(value),
                        'calculated_at': time.time()
                    }
                    converted_count += 1
                else:
                    # Invalid format - skip
                    print(f"Warning: Skipping invalid elevation cache entry: {key}")
            except Exception as e:
                print(f"Warning: Error validating elevation cache entry {key}: {e}")
        
        if converted_count > 0:
            print(f"Converted {converted_count} elevation cache entries to new format")
            # Save the cleaned cache
            try:
                self._save_elevation_cache_dict(validated_cache)
            except Exception as e:
                print(f"Warning: Could not save cleaned elevation cache: {e}")
        
        return validated_cache
    
    def _save_elevation_cache(self):
        """Save static elevation factors to cache"""
        self._save_elevation_cache_dict(self.elevation_cache)
    
    def _save_elevation_cache_dict(self, cache_dict: dict):
        """Save elevation cache dictionary to file"""
        try:
    ...
    def _get_elevation_factor(self, origin: Tuple[float, float], destination: Tuple[float, float]) -> float:
        """Get or calculate elevation efficiency factor for route segment - FIXED VERSION"""
        route_key = self._create_route_key(origin, destination)
        
        if route_key in self.elevation_cache:
            cache_entry = self.elevation_cache[route_key]
            
            # Handle both old and new cache formats safely
            if isinstance(cache_entry, dict):
                # New format: {'elevation_factor': float, 'calculated_at': timestamp}
                return cache_entry.get('elevation_factor', 1.0)
            elif isinstance(cache_entry, (int, float)):
                # Old format: just the elevation factor value
                elevation_factor = float(cache_entry)
                ...
                try:
                    ...
                    print(f"Warning: Could not save updated elevation cache: {e}")
                return elevation_factor
            else:
                # Invalid format - remove and recalculate
                print(f"Warning: Invalid elevation cache entry for {route_key}, removing")
                del self.elevation_cache[route_key]
    ...
    def _get_weather_factor(self, location: Tuple[float, float]) -> float:
        """Get weather efficiency factor for electric scooter with details storage"""
        ...
        try:
            ...
            if temp < 10:  # Cold weather affects battery chemistry
                temp_impact = 0.1
                weather_factor *= 0.9   # 10% reduction in cold
                efficiency_impacts.append(f"Cold temperature ({temp}°C): -{temp_impact*100}% efficiency")
            elif temp > 40:  # Very hot weather affects battery performance
                temp_impact = 0.05
                weather_factor *= 0.95  # 5% reduction in extreme heat
                efficiency_impacts.append(f"Hot temperature ({temp}°C): -{temp_impact*100}% efficiency")
            
            # Wind effect - scooters are more affected by wind due to lighter weight and aerodynamics
            if wind_speed > 5:  # Significant wind
                wind_impact = min(0.2, wind_speed * 0.03)  # 3% per m/s for scooters, max 20%
                wind_factor = max(0.8, 1.0 - wind_impact)
                weather_factor *= wind_factor
                efficiency_impacts.append(f"Wind ({wind_speed} m/s): -{wind_impact*100:.1f}% efficiency")
            ...
            if 'rain' in weather_data:
                ...
                efficiency_impacts.append(f"Rain conditions: -{rain_impact*100}% efficiency")
            
            # Store detailed weather information
            weather_details = {
                ...
                "overall_impact_percent": round((weather_factor - 1.0) * 100, 1)
            }
            
            # Cache the result with details
            ...
            self.conditions_cache[location_key]['weather_details'] = weather_details
...
# --- Enhanced Distance Cache with Compatibility ---
class GoogleDistanceCache:
    """Manages caching of Google Maps distance calculations with backward compatibility"""
    ...
    def _load_cache(self):
        """Load existing cache from disk with validation and cleanup"""
        try:
                ...
                with open(self.cache_file, 'rb') as f:
                    ...
                    if isinstance(cache_with_metadata, dict) and 'data' in cache_with_metadata:
                        ...
                        if cache_age_days < CACHE_EXPIRY_DAYS:
                            raw_cache_data = cache_with_metadata['data']
                            
                            # Clean and validate cache data
                            cleaned_cache = {}
                            invalid_entries = 0
                            ...
                            for key, value in raw_cache_data.items():
                                try:
                                    ...
                                    print(f"Error processing cache entry {key}: {e}")
                                    invalid_entries += 1
                            
                            self.cache_data = cleaned_cache
                            print(f"Loaded {len(self.cache_data)} cached distances (age: {cache_age_days:.1f} days)")
                            if invalid_entries > 0:
                                print(f"Cleaned up {invalid_entries} invalid cache entries")
                                # Save cleaned cache
                                self._save_cache()
                        else:
                            print(f"Cache expired (age: {cache_age_days:.1f} days), starting fresh")
                    else:
                        print("Invalid cache file format, starting fresh")
        except Exception as e:
    ...
    def get_distance(self, origin: Tuple[float, float], destination: Tuple[float, float]) -> Optional[float]:
        """Get cached distance or None if not cached - with backward compatibility"""
        ...
        if cache_key in self.cache_data:
            ...
            try:
                if isinstance(cache_entry, dict):
                    # New format: {'distance': float, 'duration': int, 'timestamp': float}
                    return cache_entry.get('distance')
                elif isinstance(cache_entry, (int, float)):
                    # Old format: just the distance value
                    distance = float(cache_entry)
                    # Convert old format to new format
                    ...
                    print(f"Warning: Unexpected cache entry type for {cache_key}: {type(cache_entry)}")
                    # Remove invalid entry
                    del self.cache_data[cache_key]
            except Exception as e:
                print(f"Error accessing cache entry {cache_key}: {e}")
...
class StrategicBatteryStrategy:
    ...
    def find_first_station(current_location: Tuple[float, float],
        ...
        """
        Find the first station that balances progress, battery utilization, and deviation from direct route.
        Returns: (station_idx, station_coords, combined_score)
        """
        
        viable_stations = []
        direct_distance = distance_cache.get_distance_with_fallback(current_location, destination)
        ...
        for station_idx, station_coords in available_stations:
            ...
            dest_efficiency = efficiency_manager.get_dynamic_efficiency(station_coords, destination)
            battery_needed_from_station = dist_station_to_dest / dest_efficiency + min_battery_reserve
            
            if battery_needed_from_station > 100.0:
                continue
            
            # --- Progress metric ---
            progress_made = max(0, direct_distance - dist_station_to_dest)
            progress_pct = (progress_made / direct_distance) * 100 if direct_distance > 0 else 0
            
            # --- Battery utilization ---
            battery_used_to_station = dist_to_station / segment_efficiency
            utilization_pct = (battery_used_to_station / current_battery) * 100
            
            # --- Deviation metric ---
            deviation = perpendicular_distance(station_coords, current_location, destination)
            # Normalize by direct_distance to get percentage
            deviation_pct = (deviation / direct_distance) * 10000 if direct_distance > 0 else 0
                        
            # --- Combined score ---
            # Prioritize: low deviation, high progress, moderate utilization
            combined_score = (-deviation_pct * 0.6) + (progress_pct * 0.2) + (utilization_pct * 0.2)
            
            # print(f"Station {station_coords}: Progress {progress_pct:.1f}%, hence the deviation {deviation_pct:.1f}%, and utilization {utilization_pct:.1f}%, Score: {combined_score:.2f}")
            
            viable_stations.append((station_idx, station_coords, combined_score, progress_pct, utilization_pct, deviation_pct))
        
        if not viable_stations:
            return None
        
        # Pick the station with the highest combined score
        best_station = max(viable_stations, key=lambda x: x[2])
        
        print(f"First station selected - Progress: {best_station[3]:.1f}%, "
            f"Battery utilization: {best_station[4]:.1f}%, "
            f"Deviation: {best_station[5]:.1f}%, Score: {best_station[2]:.2f}")
...
class EVRoutePlannerV3:
    ...
    def __init__(self,
                 ...
                 min_battery_reserve: float = 10.0,
    ...
    def find_strategic_route(self):
        ...
        while charging_stops < self.max_charging_stops:
            ...
            if charging_stops == 0:
                ...
                print("Selecting FIRST station (Minimize long detour + maximize progress + battery utilization)...")
    ...
    def generate_route_summary(self, route_tuple: Optional[Tuple[float, List[PathNode]]]) -> List[Dict]:
        ...
        for i, node in enumerate(path_nodes):
            ...
            if node.station_idx == -1:
                ...
                if charging_stations_in_route:
                    current_station_position = next((j for j, n in enumerate(charging_stations_in_route) if n == node), 0)
            ...
            # Initialize with proper default efficiency breakdown
            efficiency_breakdown = {
                "base_efficiency_km_per_percent": round(self.base_efficiency, 3),
                "elevation_factor": 1.0,
                "weather_factor": 1.0, 
                "traffic_factor": 1.0,
                "combined_efficiency_km_per_percent": round(self.base_efficiency, 3),
                "efficiency_change_percent": 0.0
            }
            
            # Initialize weather conditions as None
            weather_conditions = None
            
            # Calculate segment data if there's a next stop
            if i < len(path_nodes) - 1:
                ...
                # print(f"DEBUG: Processing segment {i}: {current_coords} -> {next_coords}")
                
                # CRITICAL FIX: Get detailed efficiency breakdown with better error handling
                try:
                    segment_efficiency = self.efficiency_manager.get_dynamic_efficiency(current_coords, next_coords)
                    # print(f"DEBUG: Got segment efficiency: {segment_efficiency}")
                    
                    # Get individual factors with detailed error handling
                    elevation_factor = 1.0
                    try:
                        elevation_factor = self.efficiency_manager._get_elevation_factor(current_coords, next_coords)
                        # print(f"DEBUG: Got elevation factor: {elevation_factor}")
                    except Exception as e:
                        print(f"WARNING: Error getting elevation factor: {e}")
                        elevation_factor = 1.0
                    
                    weather_factor = 1.0
                    try:
                        # Calculate midpoint for weather
                        midpoint = ((current_coords[0] + next_coords[0]) / 2, (current_coords[1] + next_coords[1]) / 2)
                        weather_factor = self.efficiency_manager._get_weather_factor(midpoint)
                        # print(f"DEBUG: Got weather factor: {weather_factor}")
                        
                        # Try to get detailed weather conditions
                        try:
                            midpoint_key = f"{midpoint[0]:.3f},{midpoint[1]:.3f}"
                            if (hasattr(self.efficiency_manager, 'conditions_cache') and 
                                midpoint_key in self.efficiency_manager.conditions_cache):
                                weather_cache = self.efficiency_manager.conditions_cache.get(midpoint_key, {})
                                if 'weather_details' in weather_cache:
                                    weather_conditions = weather_cache['weather_details']
                                    # print(f"DEBUG: Got weather conditions: {weather_conditions}")
                                    # Ensure weather_conditions is properly formatted
                                    if not isinstance(weather_conditions, dict):
                                        weather_conditions = None
                        except Exception as weather_detail_error:
                            print(f"WARNING: Could not get weather details: {weather_detail_error}")
                            weather_conditions = None
                            
                    except Exception as e:
                        print(f"WARNING: Error getting weather factor: {e}")
                        weather_factor = 1.0
                    
                    traffic_factor = 1.0
                    try:
                        traffic_factor = self.efficiency_manager._get_traffic_factor()
                        # print(f"DEBUG: Got traffic factor: {traffic_factor}")
                    except Exception as e:
                        print(f"WARNING: Error getting traffic factor: {e}")
                        traffic_factor = 1.0
                    
                    # CRITICAL FIX: Update efficiency breakdown with actual calculated values
                    efficiency_breakdown = {
                        "base_efficiency_km_per_percent": round(self.base_efficiency, 3),
                        ...
                        "efficiency_change_percent": round(((segment_efficiency - self.base_efficiency) / self.base_efficiency) * 100, 1)
                    }
                    
                    # print(f"DEBUG: Final efficiency breakdown: {efficiency_breakdown}")
                    
                except Exception as e:
                    print(f"ERROR: Error calculating dynamic efficiency for segment {current_coords} -> {next_coords}: {e}")
                    import traceback
                    traceback.print_exc()
                    # Use base efficiency as fallback but DON'T mark as mock
                    segment_efficiency = self.base_efficiency
                    efficiency_breakdown["combined_efficiency_km_per_percent"] = round(self.base_efficiency, 3)
                    # print(f"DEBUG: Using fallback efficiency breakdown: {efficiency_breakdown}")
                
                # Calculate battery utilization
                if battery_departure > 0 and segment_efficiency > 0:
                    battery_utilization = (next_stop_distance / segment_efficiency) / battery_departure * 100
                
                # Calculate progress made towards destination (only for source)
                if i == 0:
                    try:
                        ...
                        print(f"WARNING: Error calculating progress: {e}")
                        progress_made = 0.0
            
            # Build stop info dictionary
            stop_info = {
                ...
                # CRITICAL FIX: Always include efficiency_breakdown without conditionals
                "efficiency_breakdown": efficiency_breakdown
            }
            
            # CRITICAL FIX: Always include weather_conditions if it has actual data
            if weather_conditions and isinstance(weather_conditions, dict) and weather_conditions:
                stop_info["weather_conditions"] = weather_conditions
            
            # Add progress for source
            if progress_made > 0:
                stop_info["progress_towards_destination_km"] = round(progress_made, 2)
            
            # print(f"DEBUG: Created stop_info for {category}: has efficiency_breakdown={bool(efficiency_breakdown)}, has weather_conditions={bool(weather_conditions)}")
            
            route_summary.append(stop_info)
            
            # Update battery for next iteration
            if next_stop_distance > 0 and segment_efficiency > 0:
                current_battery = battery_departure - (next_stop_distance / segment_efficiency)

        # print(f"DEBUG: Final route summary has {len(route_summary)} stops")
        for i, stop in enumerate(route_summary):
            has_efficiency = "efficiency_breakdown" in stop and stop["efficiency_breakdown"]
            has_weather = "weather_conditions" in stop and stop["weather_conditions"]
            # print(f"  Stop {i}: {stop['category']}, efficiency_breakdown={has_efficiency}, weather_conditions={has_weather}")
...
# Add this to the end of your ev_route_service_v3.py file
...
def plan_optimal_ev_route(
    ...
    print(f"EV Route Service V3 - Dynamic Efficiency with Strategic Battery Utilization")
    ...
    try:
        ...
        if not charging_stations:
            return json.dumps({
                ...
                "route_summary": [],
                "algorithm_used": "EV Route Planner V3 (Dynamic Efficiency + Strategic Selection)",
                "google_api_calls_used": 0
            }, indent=2)

        # Create route planner with dynamic efficiency
        planner = EVRoutePlannerV3(
        ...
        if optimal_route:
            ...
            result = {
                ...
                "algorithm_used": "Dynamic Efficiency + Strategic Battery Utilization (V3)",
...
def get_service_info():
    ...
    return {
        "service_name": "EV Route Service V3",
